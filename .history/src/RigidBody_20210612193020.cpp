...
RigidBody::RigidBody(Vec2f startPos, Vec2f dimensions, Vec2f numParticles, float particleMass) :
        startPos(startPos), dimensions(dimensions) {
    initializeVariables();

    float density = particleMass / (dimensions[0]*dimensions[1]); //mass/Surface
    ...
    //Calculate Ibody
    // Ibody = M* (pow(dimensions[0],2)+pow(dimensions[0],2));
}
...
void RigidBody::recomputeAuxiliaryVars() {
    ...
    I = M* (pow(dimensions[0],2)+pow(dimensions[0],2));
    omega = Iinv * L;
}
...
void RigidBody::draw(bool drawVelocity, bool drawForce) {
    Vec2f v1 = R * Vec2f(-dimensions[0] / 2, -dimensions[1] / 2, -dimensions[2] / 2) + x;
    Vec2f v2 = R * Vec2f(dimensions[0] / 2, -dimensions[1] / 2, -dimensions[2] / 2) + x;
    Vec2f v3 = R * Vec2f(-dimensions[0] / 2, -dimensions[1] / 2, dimensions[2] / 2) + x;
    Vec2f v4 = R * Vec2f(dimensions[0] / 2, -dimensions[1] / 2, dimensions[2] / 2) + x;
    Vec2f v5 = R * Vec2f(-dimensions[0] / 2, dimensions[1] / 2, -dimensions[2] / 2) + x;
    Vec2f v6 = R * Vec2f(dimensions[0] / 2, dimensions[1] / 2, -dimensions[2] / 2) + x;
    Vec2f v7 = R * Vec2f(-dimensions[0] / 2, dimensions[1] / 2, dimensions[2] / 2) + x;
    Vec2f v8 = R * Vec2f(dimensions[0] / 2, dimensions[1] / 2, dimensions[2] / 2) + x;
    glBegin(GL_LINES);
