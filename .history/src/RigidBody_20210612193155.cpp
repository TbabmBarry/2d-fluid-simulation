...
RigidBody::RigidBody(Vec2f startPos, Vec2f coordinate, Vec2f numParticles, float particleMass) :
        startPos(startPos), coordinate(coordinate) {
    initializeVariables();

    float density = particleMass / (coordinate[0]*coordinate[1]); //mass/Surface
    //generate particles with body coordinates
    int index = 0;
    for (int x = 0; x < numParticles[0]; x++) {
        for (int y = 0; y < numParticles[1]; y++) {
            float xStart = -coordinate[0] / 2 + coordinate[0] * (float) x / (numParticles[0] - 1);
            float yStart = -coordinate[1] / 2 + coordinate[1] * (float) y / (numParticles[1] - 1);
    ...
    //Calculate Ibody
    // Ibody = M* (pow(coordinate[0],2)+pow(coordinate[0],2));
}
...
void RigidBody::recomputeAuxiliaryVars() {
    ...
    I = M* (pow(coordinate[0],2)+pow(coordinate[0],2));
    omega = Iinv * L;
}
...
void RigidBody::draw(bool drawVelocity, bool drawForce) {
    Vec2f v1 = R * Vec2f(-coordinate[0] / 2, -coordinate[1] / 2, -coordinate[2] / 2) + x;
    Vec2f v2 = R * Vec2f(coordinate[0] / 2, -coordinate[1] / 2, -coordinate[2] / 2) + x;
    Vec2f v3 = R * Vec2f(-coordinate[0] / 2, -coordinate[1] / 2, coordinate[2] / 2) + x;
    Vec2f v4 = R * Vec2f(coordinate[0] / 2, -coordinate[1] / 2, coordinate[2] / 2) + x;
    Vec2f v5 = R * Vec2f(-coordinate[0] / 2, coordinate[1] / 2, -coordinate[2] / 2) + x;
    Vec2f v6 = R * Vec2f(coordinate[0] / 2, coordinate[1] / 2, -coordinate[2] / 2) + x;
    Vec2f v7 = R * Vec2f(-coordinate[0] / 2, coordinate[1] / 2, coordinate[2] / 2) + x;
    Vec2f v8 = R * Vec2f(coordinate[0] / 2, coordinate[1] / 2, coordinate[2] / 2) + x;
    glBegin(GL_LINES);
